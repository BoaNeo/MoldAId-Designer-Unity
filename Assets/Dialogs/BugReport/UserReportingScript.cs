...
using Files;
using IO;
using Plugins.UserReporting.Scripts;
using Plugins.UserReporting.Scripts.Client;
...
namespace Dialogs.BugReport
{
    ...
    public class UserReportingScript : MonoBehaviour
    {
        ...
        /// </summary>
        public void CreateUserReport( FeatureManager features )
        {
            // Check Creating Flag
            if (this.isCreatingUserReport)
            {
...
//        UnityUserReporting.CurrentClient.TakeScreenshot(512, 512, s => { });
            ...
            UnityUserReporting.CurrentClient.CreateUserReport((br) =>
            {
                ...
                ProjectFile tempproject = new();
                features.SerializeFeatureData( tempproject );
                DataStreamJsonOutput output = new DataStreamJsonOutput();
                tempproject.Serialize(output);
            
                br.Attachments.Add(new UserReportAttachment("Project.afp", "Project.afp", "application/json", Encoding.UTF8.GetBytes($"{{{output.json}}}")));
